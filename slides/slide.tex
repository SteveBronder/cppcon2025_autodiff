...
%\setbeameroption{show notes on second screen=odd} % Both
\AtBeginSection{
...
\begin{itemize}
    \item Why you should care about Automatic Differentiation
...
\centering
% Actual 266
\animategraphics[loop,controls,scale=0.12]{6}{gif/frame}{0001}{0266}
\end{frame}
\note{
  \begin{itemize}
    \item[] Thank 3blue1brown for manim
...
\pause
\note{..}
\item Instead of N finite differences, a forward and backward pass
\pause
\note{..}
...
\caption{Log-Log Benchmark for $f$ and $f\,'$ given $f(y) = N(y | X\theta,\sigma)$}
...
\begin{itemize}
 ...
 \end{equation*}
 \item Reverse Pass: \\
 Given $z_{child}$'s adjoint (partial gradient) $\overline{z}_{child}$ \\
 Calculate the local adjoint update for $z_{parent_1}$ and $z_{parent_2}$.
 \begin{equation*}
 chain(\overline{z}_{child}, z_{parent_1}, z_{parent_2}) =
 \begin{cases}
  \overline{z}_{parent_1} \pluseq \overline{z}_{child} \cdot z{\,'}_{parent_1}\\
  \overline{z}_{parent_2} \pluseq \overline{z}_{child} \cdot z{\,'}_{parent_2}
 \end{cases}
 \end{equation*}
\end{itemize}
\end{frame}
...
\begin{frame}{What Do AD Libraries Care About?}
    \begin{itemize}
        \item Flexibility:
         \begin{itemize}
             \item Debugging, exceptions, conditional loops, matrix assignment
         \end{itemize}
         \item: Efficiency:
         \begin{itemize}
...
\note{
  \begin{itemize}
  \item[] C++ gives us the tools to meaningfully make these tradeoffs decisions.
  \item[] From Rachel: C++ is great for tools for writing high levels abstractions over lower level code 
  \end{itemize}
}
...
\note{
  ...
  \begin{itemize}
  \item[] OO is runtime and we need allocators and to think about cache locality
  \item SCT is compile time and we need to think about expression templates and template metaprogramming
  ...
  \item C++ is great for having dynamic or static
  \end{itemize}
  }
\begin{comment}
\begin{frame}{How do we keep track of our reverse pass?}
...
\end{frame}
\note{The main enemy of dynamic tapes is cache locality}
...
\begin{tikzpicture}[x=0.8cm,y=0.8cm,>=Latex,font=\small]
  ...
  \node[above right,align=left] at (0,\h+0.55) {\bfseries Noncontiguous Tape};
\end{tikzpicture}
\end{frame}
\note{
  \begin{itemize}
  \item[] This is what we want to have, but more often than not we will still end up with jumps
  \end{itemize}
}
...
\begin{tikzpicture}[x=0.8cm,y=0.8cm,>=Latex,font=\small]
  ...
  \draw[very thick,black!45] (0,-0.55) -- (0.0,-0.55) node[left=8pt] {};
  \node[below left] at (0,-0.05) {};
  \node[above right,align=left] at (0,\h+0.55) {\bfseries Contiguous Tape};
\end{tikzpicture}
...
\note{
  \begin{itemize}
  \item[] We want a big block of memory that we reuse over and over
  \item[] Same calculations are done over and over with new values
  \end{itemize}
}
...
\note{
  \begin{itemize}
    \item[] var\_impl is our base type all expression nodes inherit from
    \item[] Why vptr? Mainly optimization
  \end{itemize}
}
...
\note{
  \begin{itemize}
  \item[] Tracking everything dynamiclaly is very expensive
  \end{itemize}
}

\begin{frame}[fragile]{Source Code Transform Example:}
\begin{minted}[escapeinside=||]{C++}
double z = log(x * y);
\end{minted}
Break it down
...
\note{
  \begin{itemize}
  \item[] Fortran AD transpilers first famous for this
  \item[] We can leverage C++'s type system to do this at compile time
  \end{itemize}
}


\begin{frame}[fragile]{Source Code Transform Base Type:}
\begin{minted}[escapeinside=||]{C++}
struct var {
    double values_;
    double adjoints_;
...
\begin{frame}[fragile]{Source Code Transform Expression Node}
\begin{minted}[escapeinside=||]{C++}
...
\begin{frame}[fragile]{Source Code Transform Operator}
\begin{minted}[escapeinside=||]{C++}
template <typename T1, typename T2>
...
\begin{frame}[fragile]{Source Code Transform Expression Graph}
\begin{minted}[escapeinside=||]{C++}
...
\note{
  \begin{itemize}
  \item[] Thinking about this in polish notation lets us think about the expression templates as a stack we go through
  \end{itemize}
}

\begin{frame}[fragile]{Source Code Transform Gradient}
\begin{minted}{C++}
template <typename Expr>
inline void grad(Expr&& z) {
  adjoint(z) = 1.0;
  // Tuple nodes to be iterated in breadth-first order
...
\begin{table}
  ...
  \begin{tabular}{|l|r|c|} % Three columns: left, center, right, with vertical lines
    ...
    Method & CPU Time & Slowdown \\ % First row (headers)
...
\begin{frame}[fragile]{Source Code Transform: Downsides}
...
\note{
  \begin{itemize}
  \item[] Since the SCT needs to know how all of the nodes at compile time
  \end{itemize}
}
...
\note{
  \begin{itemize}
  \item[] For most models a large part of the time will be spent on matrix operations.
  \item[] Models like the UK covid forecasting model have large autodiff matrices
  \end{itemize}
}
...
\note{
  \begin{itemize}
  \item[] Aliasing is a bad thing that can happen in our subset assignment
  \item[] At the assign op level, we don't know if the lhs and rhs are the same object
  \end{itemize}
}
...
\end{minted}
\end{frame}
\note{Do not worry about the code here. The main thing you need to see is that subset assignment is very expensive and requires a ton of deep copies of nodes in our expression graph.}

\begin{frame}[fragile]{Source Code Transform: Matrices}
\begin{minted}{C++}
  ...
  expr.bind(std::make_pair(val_buf, 0), std::make_pair(adj_buf, 0));
\end{minted}
\end{frame}
\note{
  \begin{itemize}
  \item[] Just like the scalar source code transform we can do the same thing for matrices.
  \item[] We can also get rid of the static global tape and use a local cache instead.
  \end{itemize}
}

\begin{frame}[fragile]{Matrix Expression Graph Gradient}
\begin{minted}{C++}
template <typename Expr>
void grad(Expr&& expr, 
...
\begin{frame}[fragile]{Placeholder Node}
\begin{minted}{C++}
template <typename T>
struct Var {
  double* val_;
...
\begin{frame}[fragile]{Matrix Multiplication Expression Node}
\begin{minted}{C++}
...
\begin{frame}[fragile]{Expression Forward Pass}
\begin{minted}{C++}
template <typename... Args>
inline auto Forward(Args&&... args) {
...
\begin{frame}[fragile]{Expression Reverse Pass}
\begin{minted}{C++}
template <Matrix Op1, Matrix Op2>
struct MatMul {
  template <typename Arg>
...
\begin{frame}[fragile]{Expression Cache Size}
\begin{minted}{C++}
template <Matrix Op1, Matrix Op2>
struct MatMul {
  inline auto cache_bind_size(
    ...
    return std::make_pair(
      op1_sizes.first + op2_sizes.first + this->val_.size(),
      op1_sizes.second + op2_sizes.second + this->adj_.size());
  }
};
\end{minted}
\end{frame}
...
\begin{frame}[fragile]{Matrix Expression Graph Creation}
\begin{minted}[escapeinside=||]{C++}
  ad::Var X(4, 4); // Placeholders
  ad::Var Y(4, 4);
  auto expr = sum(X * Y);
...
\begin{figure}
\centerline{\includegraphics[scale=.5]{img/matmul_bench_slow.png}}
\label{fig-matmul1}
\end{figure}
\end{frame}
\note{
\begin{itemize}
  \item[] This is sum(X * Y) where X and Y have NxN elements
\end{itemize}
}

\begin{frame}{Matrix Multiplication Benchmark}
\begin{figure}
\centerline{\includegraphics[scale=.5]{img/matmul_bench_fast.png}}
\label{fig-matmul2}
\end{figure}
\end{frame}
\note{..}

\begin{frame}{Speedup Holds for Larger Expression Graphs}
\begin{figure}
\centerline{\includegraphics[scale=.5]{img/linux_regression_plot.png}}
\caption{Log-Log Benchmark for $f$ and $f\,'$ given $f(y) = N(y | X\theta,\sigma)$}
\label{fig-mac-red-bench2}
\end{figure}
\end{frame}
\note{
  \begin{itemize}
  \item[] As are expression graph grows the SCT matrix form continues in performance
  \end{itemize}
}
...
\note{
  \begin{itemize}
  \item[] The point is that there is still really cool research to do in autodiff
  \item[] Design decisions lead to no free lunch, but there are many options
  \end{itemize}
}
\begin{comment}
