...
\begin{tikzpicture}
  ...
  % Lines
  \path [->, draw=red, thick] (v5) -- (v1);
  \path [->, draw, thick] (v3) -- (v2);
  \path [->, draw, thick] (v4) -- (v2);
  \path [->, draw, thick] (v3) -- (v2);
  \path [->, draw, thick] (v6) -- (v4);
  \path [->, draw=red, thick] (v6) -- (v5);
  \path [->, draw, thick] (v5) -- (v3);

\end{tikzpicture}
\end{frame}

\begin{frame}{Breadthwise}
$f(x, y) = \log(x)y + \sin(x)$
\begin{figure}
  \begin{tikzpicture}
  [
...
static std::vector<vari*> vari_vec;
template <typename T>
auto& new_vari(double x) {
  return *vari_vec.emplace_back(pa.new_object<T>(x, 0.0));
}
struct vari {
  double val;
  double adj;
...
void grad(var z) {
  z.adj() = 1;
  for (auto& x : vari_vec | std::views::reverse) {
    x->chain();
  }
}
var x = 1;
...
\begin{table}
  ...
  \caption{$f(x, y) = x * \log(y) + \log(x * y) * y$;} % Table caption
