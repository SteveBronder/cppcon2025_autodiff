...
\begin{frame}{Data Tye: Expression Graph}
...
\begin{itemize}
 ...
 \begin{equation*}
 chain(\overline{v_2}, v_0, v_1) = \left\{\overline{v_0} \pluseq \diffp{v_2}{{v_0}}\overline{v_2},\overline{v_1} \pluseq \diffp{v_2}{{v_1}}\overline{v_2}\right\}
 \end{equation*}
\end{itemize}
\end{frame}
...
\end{minted}
\end{frame}
\note{The virtual function in var_impl is a performance trick}
\begin{frame}[fragile]{Operator Overloading: Simple}
\begin{minted}{C++}
void grad(var& v) {
  v.adj() = 1.0;
...
\begin{CacheLine}
  % Color some blocks
  \CacheMarkBelow{0}{var\_base\{dbl, dbl, vptr\}}
  \CacheColor{0}{green!60}
  \CacheColor{1}{green!60}
  \CacheColor{2}{green!60}
  \CacheColor{3}{red!60}
...
\begin{frame}[fragile]{Operator Overloading Approach: Matrices}
\begin{itemize}
    \item Array of Structs \mintinline{latex}{Matrix<var>}:
    \begin{itemize}
...
\pause
\begin{itemize}
    \item Struct of Arrays \mintinline{latex}{var<Matrix>}:
    \begin{itemize}
...
\begin{CacheLine}
  % Color some blocks
  \CacheMarkBelow{0}{mat\_mul\_vv\{dbl*, dbl*, vptr, lambda[var\_impl<Matrix>*, var\_impl<Matrix>*]\}}
  \CacheColor{0}{green!60}
  \CacheColor{1}{green!60}
  \CacheColor{2}{green!60}
  \CacheColor{3}{green!60}
...
\note{
We have a ton of static globals, how are supposed to run this stuff in parallel? thread_local duh!
Going to hand wave away observer model for now
}
...
\begin{figure}
\centerline{\includegraphics[scale=.5]{img/qr-code.png}}
\label{fig-qr-code}
\end{figure}
\end{frame}
